...
RubiJoint::RubiJoint(const std::string& board_name, const std::string& board_id,
                     ...
                     const std::string& nh_namespace, double scale)
    : scale_(scale),
      nh_(nh_namespace),
      home_(false),
      effort_(0.0),
  ...
  if (!get_effort_field.empty())
    eff_sub_ =
        nh_.subscribe(base_topic + "fields_from_board/" + get_effort_field, 10,
                      &RubiJoint::effortCallback, this);

  if (!get_velocity_field.empty())
    vel_sub_ =
        nh_.subscribe(base_topic + "fields_from_board/" + get_velocity_field,
                      10, &RubiJoint::velocityCallback, this);

  if (!get_position_field.empty())
    pos_sub_ =
        nh_.subscribe(base_topic + "fields_from_board/" + get_position_field,
                      10, &RubiJoint::positionCallback, this);
  ...
  if (!home_field.empty()) {
    ...
    home_sub_ = nh_.subscribe(base_topic + "fields_from_board/" + home_field, 1,
                              &RubiJoint::homeCallback, this);

    home_service_ =
        nh_.advertiseService("home", &RubiJoint::homeFunction, this);
  }
}

JointType RubiJoint::getType() { return JointType::RUBI; }
