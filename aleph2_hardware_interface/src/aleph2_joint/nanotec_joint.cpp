...
NanotecJoint::NanotecJoint(const kaco::Master& master, const uint8_t node_id,
  ...
  while (!found_device) {
    ...
    ROS_WARN_STREAM("Device with ID "
                    << (unsigned)node_id
...
void NanotecJoint::configCallback(const NanotecConfig& config, uint32_t level) {
  if (config.power) {
    ...
    if (config.brake) {
      ...
      if (op_mode_ == Nanotec::OperationMode::VELOCITY) nanotec_->SetTarget(0);
    } else {
...
JointType NanotecJoint::getType() { return JointType::NANOTEC; }

void NanotecJoint::setEffort(double effort) { setTarget(effort); }
