...
void Aleph2HW::init(const ros::NodeHandle& robot_hw_nh) {
  ...
  ROS_ASSERT(can_devices.getType() == XmlRpcValue::TypeStruct ||
             !can_devices.valid());
  ROS_ASSERT(nanotec_presets.getType() == XmlRpcValue::TypeStruct ||
             !nanotec_presets.valid());
  ...
  for (auto const& device : can_devices) {
    ...
    if (!master->start(busname, baudrate)) {
      ROS_ERROR_STREAM("failed to start can master for device "
                       << device_name << " (busname: " << busname
                       << ", baudrate: " << baudrate << ")");
      ROS_BREAK();
    }
  ...
  for (auto const& joint : hardware) {
    ...
    if (joint_struct["type"] == "rubi") {
      ...
      JointStateHandle joint_state_handle(joint_names_[i], &joint_position_[i],
                                          &joint_velocity_[i],
                                          &joint_effort_[i]);
      JointHandle joint_effort_handle(joint_state_handle,
                                      &joint_effort_command_[i]);
      JointHandle joint_velocity_handle(joint_state_handle,
                                        &joint_velocity_command_[i]);
      JointHandle joint_position_handle(joint_state_handle,
      ...
      bool has_soft_limits =
          getSoftJointLimits(joint_names_[i], robot_hw_nh, joint_soft_limits);
      ...
      if (has_limits) {
        registerLimitsHandles(joint_effort_handle, joint_velocity_handle,
                              joint_position_handle, joint_limits,
                              has_soft_limits, joint_soft_limits);
      }

      ROS_ASSERT(joint_struct.hasMember("board_name"));
      ROS_ASSERT(joint_struct["board_name"].getType() ==
                 XmlRpcValue::TypeString);
      ROS_ASSERT(joint_struct.hasMember("scale"));
      ROS_ASSERT(joint_struct["scale"].getType() == XmlRpcValue::TypeDouble);

      // optional parameters
      std::string board_id, home_field, get_effort_field, get_velocity_field,
          get_position_field, set_effort_field, set_velocity_field,
          set_position_field;
      ...
      get_effort_field =
          LoadOptionalRubiFieldFromStruct("get_effort_field", joint_struct);
      get_velocity_field =
          LoadOptionalRubiFieldFromStruct("get_velocity_field", joint_struct);
      get_position_field =
          LoadOptionalRubiFieldFromStruct("get_position_field", joint_struct);
      set_effort_field =
          LoadOptionalRubiFieldFromStruct("set_effort_field", joint_struct);
      set_velocity_field =
          LoadOptionalRubiFieldFromStruct("set_velocity_field", joint_struct);
      set_position_field =
          LoadOptionalRubiFieldFromStruct("set_position_field", joint_struct);

      joints_[i] = new aleph2_joint::RubiJoint(
          joint_struct["board_name"], board_id, get_effort_field,
          get_velocity_field, get_position_field, set_effort_field,
          set_velocity_field, set_position_field, home_field,
      ...
      ROS_ASSERT(joint_struct["can_device"].getType() ==
                 XmlRpcValue::TypeString);
      ...
      try {
        ...
        ROS_ERROR_STREAM("No can master for device "
                         << joint_struct["can_device"] << " found");
        ROS_BREAK();
      }
      ...
      if (joint_struct.hasMember("preset")) {
        ...
        if (nanotec_presets.valid() && nanotec_presets.hasMember(preset)) {
          ROS_ASSERT(nanotec_presets[preset].getType() ==
      ...
      if (joint_struct.hasMember("parameters")) {
        ROS_ASSERT(joint_struct["parameters"].getType() ==
                   XmlRpcValue::TypeStruct);
        LoadNanotecParametersFromStruct(&parameters,
                                        joint_struct["parameters"]);
      }

      joints_[i] = new aleph2_joint::NanotecJoint(
          *master, static_cast<int>(joint_struct["node_id"]),
          robot_hw_nh.getNamespace() + "/joints/" + joint_names_[i],
          static_cast<double>(joint_struct["scale"]), parameters);
    } else {
    ...
    if (joint_struct.hasMember("addons")) {
      ...
      for (int j = 0; j < joint_struct["addons"].size(); ++j) {
        ...
        if (addon["type"] == "rubi_encoder") {
          ...
          ROS_ASSERT(addon["get_position_field"].getType() ==
                     XmlRpcValue::TypeString);
          ...
          std::string board_id =
              LoadOptionalRubiFieldFromStruct("board_id", addon);

          joints_[i] = new aleph2_joint::RubiEncoderAddon(
              joints_[i], addon["board_name"], board_id,
              addon["get_position_field"], addon["scale"], addon["offset"]);
        } else {
...
void Aleph2HW::registerLimitsHandles(const JointHandle& joint_effort_handle,
  ...
  if (!has_soft_limits) {
    if (joint_limits.has_effort_limits && joint_limits.has_velocity_limits) {
      joint_limits_interface::EffortJointSaturationHandle
          joint_effort_saturation_handle(joint_effort_handle, joint_limits);
      effort_joint_saturation_interface_.registerHandle(
          joint_effort_saturation_handle);
    }
    if (joint_limits.has_velocity_limits) {
      joint_limits_interface::VelocityJointSaturationHandle
          joint_velocity_saturation_handle(joint_velocity_handle, joint_limits);
      velocity_joint_saturation_interface_.registerHandle(
          joint_velocity_saturation_handle);
    }
    if (joint_limits.has_position_limits) {
      joint_limits_interface::PositionJointSaturationHandle
          joint_position_saturation_handle(joint_position_handle, joint_limits);
      position_joint_saturation_interface_.registerHandle(
          joint_position_saturation_handle);
    }
  } else {
    if (joint_limits.has_effort_limits && joint_limits.has_velocity_limits) {
      joint_limits_interface::EffortJointSoftLimitsHandle
          joint_effort_soft_limits_handle(joint_effort_handle, joint_limits,
                                          joint_soft_limits);
      effort_joint_soft_limits_interface_.registerHandle(
          joint_effort_soft_limits_handle);
    }

    joint_limits_interface::VelocityJointSoftLimitsHandle
        joint_velocity_soft_limits_handle(joint_velocity_handle, joint_limits,
                                          joint_soft_limits);
    velocity_joint_soft_limits_interface_.registerHandle(
        joint_velocity_soft_limits_handle);

    if (joint_limits.has_velocity_limits) {
      joint_limits_interface::PositionJointSoftLimitsHandle
          joint_position_soft_limits_handle(joint_position_handle, joint_limits,
                                            joint_soft_limits);
      position_joint_soft_limits_interface_.registerHandle(
          joint_position_soft_limits_handle);
    }
  }
}
