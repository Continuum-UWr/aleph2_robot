...
#include "nanotec_driver/utils.hpp"
...
NanotecDriver::NanotecDriver(kaco::Device & device, OperationMode mode)
: device_(device), operation_mode_(mode)
{
  bool closed_loop = uint32_t(device_.get_entry("Motor drive submode select")) & (1 << 0);
  bool bldc = uint32_t(device_.get_entry("Motor drive submode select")) & (1 << 6);
  //bool hall = int32_t(device_.get_entry("Motor drive sensor display closed loop/commutation"));
...
void NanotecDriver::DowloadPowerState()
{
...
void NanotecDriver::LoadParameters(const std::map<std::string, int64_t> & parameters)
{
...
std::map<std::string, int64_t> NanotecDriver::Autocalib()
{
  std::string ret;
...
void NanotecDriver::SetPowerMode(PowerMode mode)
{
  while (true) {
    DowloadPowerState();
...
void NanotecDriver::SetVelocityProfile(uint32_t max_acceleration, uint32_t max_deceleration)
{
  device_.set_entry("Profile acceleration", max_acceleration);
  device_.set_entry("Profile deceleration", max_deceleration);
}

void NanotecDriver::SetMotorProtection(
  uint32_t nominal_current, uint32_t peak_current,
...
void NanotecDriver::SetTarget(int32_t target)
{
...
int32_t NanotecDriver::GetPosition()
{
  return device_.get_entry("Position actual value");
}

int32_t NanotecDriver::GetVelocity()
{
  return device_.get_entry("Velocity actual value");
}

int16_t NanotecDriver::GetTorque()
{
  return device_.get_entry("Torque actual value");
}

boost::optional<std::string> NanotecDriver::CheckError()
{
  std::string ret;
...
void NanotecDriver::PowerStateUp()
{
  switch (power_state_) {
    case PowerMode::OFF:
